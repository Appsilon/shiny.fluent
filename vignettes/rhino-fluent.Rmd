...
Upon viewing the default `app/main.R` file, you will notice that the code does not have any usage of `shiny.fluent` package to generate the text - "Hello!". So, let us introduce `shiny.fluent` and use it in our code to generate some text!
...
* Go to the `app/logic` folder and create a new `.R` file. Lets name it `barchart.R`.
* We shall use the `gglplot2` and `plotly` packages (which we installed in the beginning) for this purpose. 
...
The above code block loads the `ggplot2` package and generates a barchart with names of the sales reps on the X axis and the number of deals on the Y axis.

### Call the helper function in the desired `rhino` module

Now let us go back to the `app/view/datatable.R` file. Here we need to call the `generate_barchart()` function we just created along with the required dependencies to render it on UI as a plotly bar chart.

Also, we will attempt to keep the barchart beside the table as another elevated card.

So, the code is now modified to -
...
ui <- function(id) {
  ...
  div(
    ...
    Stack(
      ...
      tokens = list(padding = 20, childrenGap = 10),
      span(class = "ms-depth-8", uiOutput(ns("datatable"))),
      span(class = "ms-depth-8", plotlyOutput(ns("barchart")))
    )
  )
}
...
Similar to previous usecase, we have added `plotlyOutput` inside the `Stack` so that it is rendered horizontally beside the table as a card. It has the same styling as of the data table's card, hence no need to define the style separately. 

In the server section, we use `renderPlotly` to ask the backend of the application for rendering the barchart.
...
## Create rhino modules for toggle button and barchart

Now that we have successfully developed a working application with a data table, a toggle input and a barchart, let us further create different rhino modules for these features!

We already have the datatable module with us. We will thus create two more modules - `app/view/toggle.R` and `app/view/plot.R` along with some modifications to `app/view/datatable`. 

These modules will br called from the main file.
...
```
This module displays a text in the UI along with a toggle input item. The styling remains the same as we discussed in the previous sections.

When the user interacts with the Shiny app and changes the `includeOpen` input, the reactive expression in the server will automatically update to reflect the new value. The `moduleServer` function inside server then returns this reactive expression as its output.

### The modified `app/view/datatable.R` module

The `toggle` module when called in the main module, passes on the above discussed output to the datatable module so that the data is filtered accordingly. Hence, our `datatable` module is also modified accordingly as shown below -
...
box::use(
  dplyr[filter],
  shiny[moduleServer, NS, reactive, renderUI, span, uiOutput],
...
```
Please notice, that inside the server function, the response obtained from the `toggle` module earlier is also passed on to be used in filtering the data. The filtered data is then returned to the main module, now to be used by the `plot` module.

### The `app/view/plot.R` module

This module obtains the data from the `datatable` module and renders a barchart using the `app/logic/barchart.R` logic.
...
So far we have created the modules that we need for the app's contents. 

To make them work, we would need to call them in the main module. Here is how the main module has changed -

```
# app/view/main.R

box::use(
  shiny[moduleServer, NS, reactive, span],
...
server <- function(id) {
  moduleServer(id, function(input, output, session) {
    toggle <- toggle$server("toggle")
    data <- reactive({
      datatable$server("datatable", toggle)
    })
