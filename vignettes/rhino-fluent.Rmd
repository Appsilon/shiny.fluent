...
server <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$includeOpen, {
      filtered_deals()
    })
...
```
This module displays a text in the UI along with a toggle input item which the user can interact with. The styling remains the same as we discussed in the previous sections.

It sets up an observer in the server that watches for changes to the `includeOpen` input from the toggle. Whenever this input changes, the `filtered_deals()` reactive expression is called to update the filtered data frame and ultimately the filtered data is returned to the main module.
...
This module also obtains the data returned from the `toggle` module and renders a barchart using the `app/logic/barchart.R` logic.
