...
For the text to actually appear, we need to put it inside a
[`withReact`](https://appsilon.github.io/shiny.react/reference/withReact.html) function call
which takes care of rendering Fluent components in user's browsers using React.
Use `?fluentPage` to create the UI, as it will add proper CSS classes and suppress Bootstrap
...
`fluent_sales_data` is a data frame of randomly generated "sales deals", which are assigned to one of the `fluentPeople`, have a date and an amount, and are associated with one of the top 10 companies from the Fortune 500 list (including its name, city and map coordinates).
...
They are shiny.fluent inputs and behave in a very similar way to regular Shiny inputs:
they take an ID as the first argument, and optionally an initial value.
...
```{r, echo=FALSE}
knitr::include_graphics("images/tutorial-part1-final.gif")
```
