...
ui <- fluidPage(
  div(
    ...
    Checkbox.shinyInput("basicInput", value = TRUE),
...
server <- function(input, output, session) {
  ...
  output$dynamicUi <- renderUI({
    if (show()) {
      div(
        ...
        Checkbox.shinyInput("checkbox", value = isolate(isTRUE(input$checkbox))),
        textOutput("checkboxValue"),

        h4("Slider"),
        Slider.shinyInput("slider", value = 0, min = -100, max = 234),
  ...
  observeEvent(input$updateInputs, {
    updateCheckbox.shinyInput(session, "checkbox", value = !input$checkbox)
    updateSlider.shinyInput(session, "slider", value = input$slider + 10)
    updateTextField.shinyInput(session, "textField", value = paste(input$textField, "new text"))
  })
}
